...
function okToRun(req: NextRequest): boolean {
  ...
  if (secret && provided && provided === secret) return true; // manual authorized
  if (process.env.NODE_ENV !== 'production') return true; // allow locally
  return false;
}
...
function renderEmailHtml(opts: {
  ...
  kpis: { total: number; low: number; over: number; worst?: { sku: string; name: string; daysLeft: number } | null };
  lowTop: { sku: string; name: string; qtyOnHand: number; safetyStock: number; daysLeft: number | null }[];
  overTop: { sku: string; name: string; qtyOnHand: number; safetyStock: number; outQty_30: number }[];
  dashboardUrl?: string | null;
  ai?: { executiveSummary: string | null; insights: string[]; actions: string[] } | null;
  draft?: { sku: string; name: string; reason: string; priority: 'High'|'Medium'; recommended: number }[];
}): string {
  const { windowDays, kpis, lowTop, overTop, dashboardUrl, ai, draft } = opts;
  const dt = fmtJakarta(new Date());
  const link = dashboardUrl || process.env.APP_BASE_URL ? `${process.env.APP_BASE_URL}/dashboard/inventory-health` : '/dashboard/inventory-health';

  const lowRows = lowTop.map((i) => `
    <tr>
      <td style="padding:6px 8px;border-bottom:1px solid #eee;font-family:Inter,Arial,sans-serif;font-size:12px">${htmlEscape(i.sku)}</td>
      <td style="padding:6px 8px;border-bottom:1px solid #eee;font-family:Inter,Arial,sans-serif;font-size:12px">${htmlEscape(i.name)}</td>
      <td style="padding:6px 8px;border-bottom:1px solid #eee;text-align:right;font-family:Inter,Arial,sans-serif;font-size:12px">${i.qtyOnHand}</td>
      <td style="padding:6px 8px;border-bottom:1px solid #eee;text-align:right;font-family:Inter,Arial,sans-serif;font-size:12px">${i.safetyStock}</td>
      <td style="padding:6px 8px;border-bottom:1px solid #eee;text-align:right;font-family:Inter,Arial,sans-serif;font-size:12px">${i.daysLeft !== null ? Math.max(0, Math.floor(i.daysLeft)) : '—'}</td>
    </tr>`).join('');

  const overRows = overTop.map((i) => `
    <tr>
      <td style="padding:6px 8px;border-bottom:1px solid #eee;font-family:Inter,Arial,sans-serif;font-size:12px">${htmlEscape(i.sku)}</td>
      <td style="padding:6px 8px;border-bottom:1px solid #eee;font-family:Inter,Arial,sans-serif;font-size:12px">${htmlEscape(i.name)}</td>
      <td style="padding:6px 8px;border-bottom:1px solid #eee;text-align:right;font-family:Inter,Arial,sans-serif;font-size:12px">${i.qtyOnHand}</td>
      <td style="padding:6px 8px;border-bottom:1px solid #eee;text-align:right;font-family:Inter,Arial,sans-serif;font-size:12px">${i.safetyStock}</td>
      <td style="padding:6px 8px;border-bottom:1px solid #eee;text-align:right;font-family:Inter,Arial,sans-serif;font-size:12px">${i.outQty_30}</td>
    </tr>`).join('');

  const aiSection = (() => {
    if (!ai) return '';
    const exec = ai.executiveSummary ? `<p style=\"margin:6px 0;color:#111;font-size:13px\">${htmlEscape(ai.executiveSummary)}</p>` : '';
    const insights = (ai.insights || []).slice(0, 5).map((s) => `<li>${htmlEscape(s)}</li>`).join('');
    const actions = (ai.actions || []).slice(0, 5).map((s) => `<li>${htmlEscape(s)}</li>`).join('');
    return `
      <h3 style=\"margin:18px 0 6px 0;font-size:14px\">AI Insights</h3>
      ${exec}
      ${insights ? `<div style=\"margin-top:6px\"><div style=\"color:#666;font-size:12px\">Insights:</div><ul style=\"margin:6px 0 0 18px;font-size:12px;color:#111\">${insights}</ul></div>` : ''}
      ${actions ? `<div style=\"margin-top:6px\"><div style=\"color:#666;font-size:12px\">Actions:</div><ul style=\"margin:6px 0 0 18px;font-size:12px;color:#111\">${actions}</ul></div>` : ''}
    `;
  })();

  const draftRows = (draft || []).map((r) => `
    <tr>
      <td style=\"padding:6px 8px;border-bottom:1px solid #eee;font-family:Inter,Arial,sans-serif;font-size:12px\">${htmlEscape(r.sku)}</td>
      <td style=\"padding:6px 8px;border-bottom:1px solid #eee;font-family:Inter,Arial,sans-serif;font-size:12px\">${htmlEscape(r.name)}</td>
      <td style=\"padding:6px 8px;border-bottom:1px solid #eee;text-align:center;font-family:Inter,Arial,sans-serif;font-size:12px\"><span style=\"display:inline-block;padding:2px 6px;border-radius:10px;background:${r.priority==='High'?'#fee2e2':'#fef9c3'};color:${r.priority==='High'?'#991b1b':'#92400e'}\">${r.priority}</span></td>
      <td style=\"padding:6px 8px;border-bottom:1px solid #eee;text-align:right;font-family:Inter,Arial,sans-serif;font-size:12px\">${r.recommended}</td>
      <td style=\"padding:6px 8px;border-bottom:1px solid #eee;font-family:Inter,Arial,sans-serif;font-size:12px\">${htmlEscape(r.reason)}</td>
    </tr>`).join('');
  ...
  <div style="font-family:Inter,Arial,sans-serif;color:#111">
    ...
    <div style="margin-top:12px;padding:10px;border:1px solid #e5e7eb;border-radius:8px">
      <div style="display:flex;gap:16px;flex-wrap:wrap">
        <div><div style="color:#666;font-size:12px">Total Analyzed</div><div style="font-size:18px;font-weight:600">${kpis.total}</div></div>
        <div><div style="color:#666;font-size:12px">Low Stock</div><div style="font-size:18px;font-weight:600">${kpis.low}</div></div>
        <div><div style="color:#666;font-size:12px">Overstock</div><div style="font-size:18px;font-weight:600">${kpis.over}</div></div>
        ${kpis.worst ? `<div><div style=\"color:#666;font-size:12px\">Worst Risk</div><div style=\"font-size:14px;font-weight:600\">${htmlEscape(kpis.worst.name)} (${htmlEscape(kpis.worst.sku)}) • ~${Math.max(0, Math.floor(kpis.worst.daysLeft))} hari</div></div>` : ''}
      </div>
    </div>
    ...
    <table style="width:100%;border-collapse:collapse">
      ...
      <tbody>${lowRows || `<tr><td colspan=\"5\" style=\"padding:8px;color:#666\">—</td></tr>`}</tbody>
    </table>
    ...
    <table style=\"width:100%;border-collapse:collapse\">
      ...
      <tbody>${draftRows || `<tr><td colspan=\"5\" style=\"padding:8px;color:#666\">—</td></tr>`}</tbody>
    </table>
    ...
    <table style="width:100%;border-collapse:collapse">
      ...
      <tbody>${overRows || `<tr><td colspan=\"5\" style=\"padding:8px;color:#666\">—</td></tr>`}</tbody>
    </table>
...
async function sendEmailSMTP({ subject, html }: { subject: string; html: string }) {
  ...
  if (!host || !user || !pass || !from || !to) {
    throw new Error('SMTP env vars missing (SMTP_HOST, SMTP_USER, SMTP_PASS, EMAIL_FROM, EMAIL_TO)');
  }

  const transporter = nodemailer.createTransport({ host, port, secure, auth: { user, pass } });
  const recipients = to.split(',').map((s) => s.trim()).filter(Boolean);
  const info = await transporter.sendMail({ from, to: recipients, subject, html, text: 'Lihat versi HTML untuk ringkasan lengkap.' });
...
async function runJob(windowDays: DaysWindow = 30) {
  ...
  try {
    ...
    ai = {
      ...
      insights: [
        res.lowStock.length ? 'Ada item stok menipis/risiko stockout; perlu perhatian cepat.' : 'Tidak ada stok menipis signifikan.',
        res.overStock.length ? 'Sejumlah item berlebih; optimalkan ruang dan modal.' : 'Tidak ada overstock signifikan.'
      ],
      actions: [
  ...
  function getAutoParams(item: ComputedItem): { lead: number; buffer: number } {
    ...
    let lead = 7;
    let buffer = 3;
    if (avg30 <= 0 && avg7 <= 0) buffer = 2; else if (avg7 > avg30 * 1.3) buffer = 5; else buffer = 3;
    return { lead, buffer };
  }
  function computeRecommendedQty(item: ComputedItem): number {
    ...
    if (isStockout) targetLevel = (item.safetyStock ?? baseTarget) + avg * (Math.max(0, lead) + Math.max(0, buffer));
  ...
  const html = renderEmailHtml({ windowDays, kpis, lowTop, overTop, dashboardUrl: process.env.APP_BASE_URL || null, ai, draft });
  const subject = `Inventory Health Report • ${fmtJakarta(new Date())} WIB`;
  const messageId = await sendEmailSMTP({ subject, html });
  return { messageId, kpis, counts: { low: lowTop.length, over: overTop.length } };
}

export async function GET(req: NextRequest) {
  if (!okToRun(req)) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  try {
